...
def main():
    ...
    chunk_size=2000
    shakefile = open('input.txt', "r")
    mobyfile = open('mobydick.txt', "r")
    shakestr = shakefile.read()
    mobystr = mobyfile.read()
    clean = ''
    count=0
    full_str = ''
    list_of_point_names=[]
    ...
    list_of_points_before_relatin_maping=[]
        ...
        # for symbol in ["\n",':','.',"'",'&','-']:
    lineclean = re.sub(r'[^\w\s]', '', mobystr)
    lineclean = lineclean.replace("\n", " ")
    lineclean = lineclean.lower()
    mobyclean = clean + lineclean

    full_str = full_str + mobyclean
    #print(full_str)
    lineclean = re.sub(r'[^\w\s]', '',shakestr)
    lineclean = lineclean.replace("\n", " ")
    lineclean = lineclean.lower()
    shakeclean = clean + lineclean
    full_str = full_str + shakeclean

    point=string_to_datapoint_without_relations('mobydick',mobyclean)
    list_of_points_before_relatin_maping.append(point)
    list_of_point_names.append(point.unique_name)


    point = string_to_datapoint_without_relations('shaky', shakeclean)
    list_of_points_before_relatin_maping.append(point)
    list_of_point_names.append(point.unique_name)


    mat_dict= string_to_dict(full_str,2)
    bob=sorted(mat_dict.matrix,key=lambda x: x[1])

    #print("267",bob)
    bizdismap=list_of_poins_to_map(list_of_points_before_relatin_maping, mat_dict)
    print(list_of_points_before_relatin_maping)
    #print( bizdismap,"240")

    dictfromdp= list_of_data_points_to_dict(list_of_points_before_relatin_maping)

    #print(searchalgo('boat',list_of_points_before_relatin_maping,mat_dict,bizdismap),340)
